...
"
" Modified 7/23/13 - BS - include entity work.component_name in instantiations.
" This eliminates the need for component declaration.
" Modified 1/28/14 - BS - when copying an entity and pasting as an instance,
" signal name will be the port name.  If the port name starts with i_, o_, or
" b_, then remove this prefix from the signal name.
"
" Modified 3/3/14 - BS - add a signal width comment when pasting an instance.
" This only works for std_logic and std_logic_vector
...
if !exists("g:sigPrefix_VIP")
  let g:sigPrefix_VIP = ""            " the prefix added to signals names
endif
...
" modification to include entity work.component_name in instantiations.  This
" eliminates the need for component declaration -- BS 7/23/13
  ...
  try
    ...
    for i in range(0, nbOfLines)
	  ...
	  "BS added signal width
	  if match(currentLine, "std_logic_vector") >= 0
"		  let signalWidth = substitute(currentLine, "^.*\(\\s*", "", "g") " remove everything up to the vector #
		  let signalWidth = substitute(currentLine, "^.*std_logic_vector\s*", "", "g") " remove everything up to the vector #
"		  let signalWidth = substitute(signalWidth, "\\s.*$", "", "g")
		  let signalWidth = substitute(signalWidth, "[\^\)]*$", "", "")
	  elseif match(currentLine, "std_logic") >= 0
		  let signalWidth = "std_logic"
	  else
		  let signalWidth = "error" 
	  endif
      ...
      for currentWord in currentList
        ...
        if (braceCnt > 0) && (skipLine == 0) && (k == 0)
          ...
          if inPort == 1
			 " BS remove i_, o_, b_ if it precedes the port name 
			  if strpart(signalName,0,2) ==? "i_" || strpart(signalName,0,2) ==? "o_" || strpart(signalName,0,2) ==? "b_"
			  let instanceBlock += [signalBefore." => ".a:sigPrefix.strpart(signalName,2).", -- ".signalWidth]
			else
			  let instanceBlock += [signalBefore." => ".a:sigPrefix.signalName.", -- ".signalWidth]
			endif
          endif
        endif
